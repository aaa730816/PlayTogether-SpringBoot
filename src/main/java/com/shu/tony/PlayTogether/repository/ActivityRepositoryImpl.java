...
public class ActivityRepositoryImpl {
    ...
    public PageImpl<ActivityVo> getNearest(ActivityCriteria activityCriteria) {
        ...
        query.setParameter("latidude", Double.valueOf(activityCriteria.getGeoLocation().getLatitude()));
        query.setParameter("longitude", Double.valueOf(activityCriteria.getGeoLocation().getLongitude()));
        query.setParameter("activityType", activityCriteria.getActivityType());
        Query countQuery = entityManager.createNativeQuery("SELECT count(*) from activity");
